                ...
                using (BinaryReader br = new BinaryReader(fileStream))
                {
                    ...
                    if (msgSuported == MessageBoxResult.Yes) {
                        ...
                        for (var i = 0; i < MAX_VRAM_ENTRIES; i++) {
                            var offset = atom_vram_entry_offset + 0x3D;
                            if (bMultiVRAM)
                            {
                                var diff = i == 0 ? 0x86 : vramTimingOffset;
                                offset = atom_vram_entry_offset + diff;
                            }
                            ...
                            if (atom_vram_timing_entries[i].ulClkRange != 0) continue;
